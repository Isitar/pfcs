...
public class WinkelBeschleun
    ...
    double m = 1;
    double I = m * len * len / 3; // Trägheitsmoment
    double phi = 89.0 * Math.PI / 180.0; // winkel
    double omega = 0; // winkelgeschwindigkeit

    double[] xBrett = {phi, omega};
    ...
    private Consumer<double[]> calcWinkelBeschl = (double[] x) -> {
        ...
        //double omega = x[1];
        double x0 = x[1];
        double x1 = -len / 2 * m * g * Math.cos(x[0]) / I;
        x[0] = x0;
        x[1] = x1;
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        zeichneStrecke(gl, 0, 0, (float) (len * Math.cos(xBrett[0])), (float) (len * Math.sin(xBrett[0])));

        /*double len = 1.1; // Brett
        double m = 1;
        double I = m * len * len / 3; // Trägheitsmoment
        double phi = 30.0 * Math.PI / 180.0; // winkel
        double omega = 0; // winkelgeschwindigkeit
        double Ft = m * g * Math.cos(phi);
         */
 ...
 /*
        ...
        omega -= (len / 2) * Ft / I * dt;
         */
        xBrett = ch.fhnw.pfcs.helpers.Dynamics.rungeKutta(xBrett, dt, calcWinkelBeschl);
    }
