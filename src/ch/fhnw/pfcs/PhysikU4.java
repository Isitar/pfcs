...
public class PhysikU4 implements WindowListener, GLEventListener, KeyListener, FigureHolder {
	...
	void createFrame() // Fenster erzeugen
	{
		...
		JOptionPane pane = new JOptionPane("r/R = Radius verändern\r\ni/I=Strichbreite verändern");
	...
	public void display(GLAutoDrawable drawable) {
		...
		mygl.rewindBuffer(gl);
		mygl.setColor(0, 1, 0);
		double deltaAbstand = 0.2;
		for (int i = 0; i < 50; ++i) {
			drawLine(gl, mygl, -boundaryX, deltaAbstand * i, noOfSteps, 0.1);
			drawLine(gl, mygl, -boundaryX, -deltaAbstand * i, noOfSteps, 0.1);
		}
		currTime++;
	}
	...
	public void keyTyped(KeyEvent e) {
		try {
			...
			if (e.getKeyChar() == 'R') {
				cilinderRadius -= 0.5;
				cilinderRadius = Math.max(cilinderRadius, 0);
				if (cilinderRadius > 0) {
					noOfSteps -= 20;
				}
			}
			...
			if (e.getKeyChar() == 'I') {
				...
				modulLength -= 2;
				modulLength = Math.max(modulLength, 2);
			}
