...
public class Stoss2D implements WindowListener, GLEventListener, KeyListener {
	...
	class Kugel {
		 ...
		 *            Geschwindigkeit Y
		 */
		public Kugel(double r, double m, double xm, double ym, double vx, double vy) {
			this.xm = xm;
			this.ym = ym;
			this.vx = vx;
			this.vy = vy;
	 ...
	 * @return double[] {v1',v2'}
	 */
	private double[] stoss1D(double m1, double v1, double m2, double v2, double k) {
		return new double[] { (m1 * v1 + m2 * v2 - (v1 - v2) * m2 * k) / (m1 + m2),
	...
	public void reshape(GLAutoDrawable drawable, int x, int y, int width, int height) {
		...
		ytop = aspect * xright;
		P = Mat4.ortho((float) xleft, (float) xright, (float) ybottom, (float) ytop, (float) znear, (float) zfar);
		mygl.setP(gl, P);
	}
